 ...
 * $tree=new Tree();
 * //生成下拉菜单形式
 * $options=$tree->getTreeOptions($list);
 *
 */
    ...
    public function getTree($list, $pid = 0)
    {
        ...
        if (!empty($list)) {
            ...
            foreach ($newList as $value) {
                if ($pid == $value[$this->pidName]) {//先取出顶级
                    $tree[] = &$newList[$value[$this->idName]];
                } elseif (isset($newList[$value[$this->pidName]])) {//再判定非顶级的pid是否存在，如果存在，则再pid所在的数组下面加入一个自定义键，来将本身存进去
